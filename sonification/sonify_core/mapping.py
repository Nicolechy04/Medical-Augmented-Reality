"""Map OCT image classes and intensities to physical sound-model parameters."""
...
# Create per-node mass, stiffness, and damping arrays for the initial model.
...
def remap_classwise(values, classes, range_params_all, param_index, scale="linear", invert=False):
    """Normalize values within each tissue class and map them to its configured range."""
    ...
    for c in np.unique(classes):
        ...
        # direction = "↑" if vmax_target > vmin_target else "↓"
        # print(f"Class {c} ({direction}{' inv' if invert else ''}): "
        #       f"[{remapped_c.min():.5f}, {remapped_c.max():.5f}] "
        #       f"→ target [{vmin_target}, {vmax_target}]")
