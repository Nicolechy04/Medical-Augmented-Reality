...
from extrapolate import extrapolate_retina, thicken_rpe_down
...
class OfflineInitialData:
    """Normalized first-frame data and aligned frame/segmentation file lists."""
    frame_files: list
    seg_files_path: str
    seg_files: list
    frame_0: np.ndarray
...
class InitialGeometry:
    """Reference anatomy, rotation, needle position, and ROI from the first frame."""
    frame_0: np.ndarray
    seg_img_0: np.ndarray
    ilm_line: np.ndarray
    rpe_line: np.ndarray
...
class SoundModelSetup:
    """Fully initialized node grid and its mapped physical parameters."""
    roi: tuple | None
...
def spline_fitting_worker(input_queue, output_queue, stop_event=None, debug=False):
    """Fit ILM/RPE splines asynchronously and return the newest usable geometry.

    Bounded queues deliberately drop work when processing falls behind, keeping
    the live pipelines responsive instead of accumulating stale frames.
    """
...
class BaseIOCTSonification:
    """Common state and operations shared by distance and physical sonification."""
    ...
    def log_info(self, message):
        """Log through an optional host logger, otherwise print to the console."""
    ...
    def log_warn(self, message):
        """Emit a warning through an optional host logger or the console."""
        logger = self._get_logger()
        if logger is not None:
            logger.warn(message)
        else:
    ...
    def log_error(self, message):
        """Emit an error through an optional host logger or the console."""
        logger = self._get_logger()
        if logger is not None:
    ...
    def _get_logger(self):
        """Return a logger supplied by a subclass or embedding application."""
    ...
    def reset_tracking_state(self):
        """Clear values that carry needle tracking between frames or runs."""
        self.prev_tip_pos = None
        self.prev_needle_pos_for_speed = None
        self.prev_def_amount = 0
    ...
    def prepare_offline_initial_data(
        ...
        *,
        ...
        """Discover aligned inputs and normalize the first frame and segmentation.

        Every selected OCT frame must have a segmentation. ``fallback_loader``
        remains a generic extension point, but the cleaned pipelines pass
        ``None`` and therefore never run inference.
        """
        ...
        if len(seg_files) != len(frame_files):
            raise FileNotFoundError(
                "Every selected OCT frame must have a segmentation. "
                f"Found {len(frame_files)} frames and "
                f"{len(seg_files)} segmentations."
            )
        ...
        if seg_img_0 is None:
            raise ValueError(
                f"Could not read initial segmentation: {seg_0_path}"
            )
    ...
    # Unity/Subretinal dataset compatibility:
    # The Unity visualization dataset stores one 3D OCT volume per time frame:
    #   <dataset>/iOCT Microscope/Volume/00000/00000.png ... 00511.png
    # The existing sonification pipeline expects one 2D image per time frame.
    # To keep the rest of the pipeline unchanged, we expose one consistent
    # representative B-scan slice from each 3D volume as the 2D frame sequence.
    @staticmethod
    def _subretinal_volume_root(folder):
        """Return the new dataset's volume root when its expected layout exists."""
        volume_root = os.path.join(folder, "iOCT Microscope", "Volume")
        return volume_root if os.path.isdir(volume_root) else None

    @staticmethod
    def _select_subretinal_bscan_slice(time_frame_dir, preferred_slice="00256.png"):
        """Choose the preferred central B-scan, or the middle available slice."""
        preferred_path = os.path.join(time_frame_dir, preferred_slice)
        if os.path.exists(preferred_path):
            return preferred_slice

        candidates = sorted(
            f for f in os.listdir(time_frame_dir)
            if f.endswith((".png", ".jpg")) and os.path.isfile(os.path.join(time_frame_dir, f))
        )
        if not candidates:
            return None
    ...
    def list_frame_files(folder, suffixes=(".jpg", ".png")):
        """List one representative B-scan per time point, preserving time order."""
        volume_root = BaseIOCTSonification._subretinal_volume_root(folder)
        if volume_root is not None:
            frame_files = []
            time_frame_dirs = sorted(
                d for d in os.listdir(volume_root)
                if os.path.isdir(os.path.join(volume_root, d))
            )

            for time_frame in time_frame_dirs:
                time_frame_dir = os.path.join(volume_root, time_frame)
                slice_name = BaseIOCTSonification._select_subretinal_bscan_slice(time_frame_dir)
                if slice_name is not None:
                    frame_files.append(
                        os.path.join("iOCT Microscope", "Volume", time_frame, slice_name)
                    )
    ...
    def list_segmentation_files(folder, seg_folder_suffix="_segmentation", predicate=None):
        """Build a segmentation list aligned one-to-one with the selected frames."""
        volume_root = BaseIOCTSonification._subretinal_volume_root(folder)
        if volume_root is not None:
            seg_files_path = folder
            seg_files = []
            time_frame_dirs = sorted(
                d for d in os.listdir(volume_root)
                if os.path.isdir(os.path.join(volume_root, d))
            ...
            for time_frame in time_frame_dirs:
                ...
                seg_abs_path = os.path.join(folder, seg_rel_path)

                if not os.path.exists(seg_abs_path):
                    raise FileNotFoundError(
                        f"Missing segmentation for volume {time_frame}: {seg_abs_path}"
                    )

                if predicate is None or predicate(os.path.basename(seg_rel_path)):
                    seg_files.append(seg_rel_path)
    ...
    def load_segmentation_from_source(self, seg_file=None, *, fallback_loader=None, default_confidence=1.0):
        """Load and normalize a label mask, optionally using a supplied fallback."""
        seg_img = None
    ...
    def load_frame_at_index(self, folder, frame_files, index):
        """Read one frame and apply the resolution established during initialization."""
    ...
    def load_runtime_segmentation(self, seg_file=None, *, fallback_loader=None, cls_to_use=None):
        """Load one mask and optionally fill the region between retinal layers."""
    ...
    def normalize_frame(self, frame, add_margin_to_roi=None):
        """Resize a frame and scale its ROI margin to the same coordinate system."""
        self.scale_x = 1.0
        self.scale_y = 1.0
        resized_frame = frame
    ...
    def normalize_segmentation(self, seg_img):
        """Resize labels with nearest-neighbor interpolation to preserve class IDs."""
        if seg_img is None:
            return None
    ...
    def prepare_initial_segmentation(self, seg_img, *, rpe_thickness_pixels):
        """Fit initial retinal boundaries and establish spline/thickness baselines.

        Class 4 is regenerated between ILM and RPE, while the original needle
        mask is restored afterward so it is not overwritten by retinal filling.
        """
        seg_img = seg_img.copy()
        seg_img[seg_img == 4] = 0
    ...
    def prepare_initial_geometry(
        ...
        *,
        ...
        """Create the complete first-frame anatomical coordinate system."""
    ...
    def compute_rotation(self, frame, seg_img, *, use_huber_regressor=False):
        """Rotate the OCT so the RPE is approximately horizontal.

        Robust Huber fitting can reduce the influence of mislabeled boundary
        pixels. If too few RPE pixels exist, the identity transform is used.
        """
    ...
    def build_line_points(self, seg_img, frame_shape, needle_tip_pos, *, injection_mode):
        """Represent the needle axis used to orient and position the model ROI."""
    ...
    def compute_roi(
        ...
        *,
        ...
        """Find and adjust the anatomical ROI around the transformed needle line."""
        if not line_points:
            return None, None
    ...
    def roi_contains_layers(seg_rotated, roi, *, layers=(2, 3)):
        """Check whether all requested anatomical labels occur inside an ROI."""
        if roi is None:
            return False, None, {}
    ...
    def weighted_patch_classification(patch):
        """Assign a patch class while giving thin ILM/RPE labels extra weight."""
        if patch.size == 0:
            return 0
    ...
    def compute_patch_center_classes(
        ...
        *,
        ...
        """Classify every model node from the segmentation around its center.

        Post-processing ensures that thin anatomical layers remain represented
        after the dense segmentation is reduced to the much coarser node grid.
        """
    ...
    def find_layer_bounds(self, patch_centers_class):
        """Find the first ILM and RPE node rows along the grid's center column."""
        ilm_bound = None
        rpe_bound = None
    ...
    def build_node_info(self, rotated_patch_centers, ilm_line, rpe_line, *, eps=2.0):
        """Store each node's anatomical relationship for later geometry updates.

        Retina nodes retain their fractional depth between ILM and RPE; nodes
        outside the retina retain their signed offset from the nearest boundary.
        """
    ...
    def initialize_sound_model_setup(
        ...
        *,
        ...
        """Build, classify, parameterize, and configure the physical node model."""
        roi = geometry.roi
        if roi is None:
    ...
    def swap_range_layers_and_compute_scales(self, range_params_all):
        """Adapt configured class ranges and derive bounded per-class force scales."""
    ...
    def start_spline_worker(self, debug=False):
        """Start the bounded background worker used for live spline fitting."""
    ...
    def queue_spline_task(self, frame_index, seg_img_current):
        """Queue a mask without blocking; return ``False`` when the worker is behind."""
    ...
    def collect_recent_spline_results(self, frame_index, *, max_age=10):
        """Drain completed fits and discard results older than ``max_age`` frames."""
        results = []
    ...
    def clear_spline_queues(self):
        """Discard queued spline inputs and outputs while keeping queue counts valid."""
    ...
    def stop_spline_worker(self, timeout=2.0):
        """Signal the spline worker and wait briefly for it to terminate."""
    ...
    def start_simulator(self, *, suppress_output=False, new_process_group=False):
        """Launch SonoBox, or assume an external instance when no path is configured."""
        if not self.simulator_path:
            print("⚠️ No simulator path provided - assuming SonoBox already running")
            return None
        kwargs = {}
        if suppress_output:
    ...
    def show_startup_preview(frame, *, wait_ms=0, window_name="Press any key to start sonification"):
        """Display the initialized ROI before entering the live processing loop."""
        cv2.imshow(window_name, frame)
        cv2.waitKey(wait_ms)
        cv2.destroyAllWindows()

    def filter_tracked_tip(self, tracked_tip, *, max_jump=None):
        """Reject implausible frame-to-frame needle jumps by keeping the last tip."""
    ...
    def find_closest_node_and_class(self, needle_tip_pos, rotated_patch_centers, patch_centers_class):
        """Find the closest grid row and return its center-column anatomical class.

        The signed distance is negative when the tip lies below the selected node.
        Using the center column makes class lookup stable across lateral motion.
        """
    ...
    def snap_to_nearest_class_row(self, needle_tip_pos, rotated_patch_centers, class_mask):
        """Snap a tip to the center node of the nearest row matching ``class_mask``."""
    ...
    def apply_spline_result_to_patch_centers(self, result, rotated_patch_centers, node_info, current_class, *, gamma):
        """Move nodes with updated boundaries while preserving anatomical roles.

        ``gamma`` controls how strongly each node follows its new target. Retina
        nodes preserve relative depth; boundary and exterior nodes preserve their
        original boundary relationship.
        """
    ...
    def smooth_force_values(self, new_f_ilm, new_f_rpe, *, alpha=0.2, decay_rate=0.02):
        """Smooth boundary forces and gradually decay inactive cues toward zero."""
    ...
    def calculate_ramp_intensity(*, is_active, ramp_counter, sonification_rate, ramp_duration, min_intensity=0.15, max_intensity=1.0):
        """Return a smoothstep fade-in multiplier for an active audio cue."""
        if not is_active:
            return 0.0
    ...
    def update_ramp_state(current_active, ramp_counter, requested_active):
        """Advance or reset the sample counter used by the cue fade-in."""
    ...
    def create_class_change_record(frame_index, prev_class, current_class, needle_tip_pos, start_time):
        """Create timestamped metadata for one anatomical class transition."""
    ...
    def buffer_video_frame(video_frames_buffer, frame, video_start_time, *, audio_started, start_audio_callback):
        """Buffer a timestamped frame and start audio at the shared time origin."""
