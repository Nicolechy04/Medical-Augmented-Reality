"""Send model parameters, forces, and recording commands to Processing/SonoBox."""
...
# Shared OSC connection used by the physical-model pipeline.
...
def sonify(intensity_range, averaged_values, force):
    ...
    # target_range = update_target_ranges(len(intensity_range))
    ...
    # print("min max for transfer function: {}".format(intensity_range))

    for line in profile:
        label = line[1]//mass_per_bucket

        #map_oscMass = interp1d(np.exp(intensity_range), np.exp(target_ranges[int(label)][0]), kind='linear')
        #mass = exp_interplt(line[3], map_oscMass)
        ...
        # print(f'mass data: {line}, {mass}')

    for spdmp in springList:
        node1, node2 = profile[spdmp[0]], profile[spdmp[1]]
        label = node1[1]//mass_per_bucket

        #map_sprStf = interp1d(np.exp(intensity_range), np.exp(target_ranges[int(label)][1]), kind='linear')
        map_sprStf = interp1d(intensity_range, target_ranges[int(label)][1], kind='slinear', fill_value='extrapolate')
        #map_sprDmp = interp1d(np.exp(intensity_range), np.exp(target_ranges[int(label)][2]), kind='linear')
        ...
        #f_sprStf_dec3 = np.round(exp_interplt(averaged_values[int(label)], map_sprStf), 3)

        f_sprDmp_dec4 = np.round(map_sprDmp(averaged_values[int(label)]), 4)
        #f_sprDmp_dec4 = np.round(exp_interplt(averaged_values[int(label)], map_sprDmp), 3)
        # print('for mean value: {}, stiffness: {}, damping: {}'.format(averaged_values[int(label)], f_sprStf_dec3, f_sprDmp_dec4))
...
def sonify_ascan(forces, separate_f_components=False, ramp=False):
    """Send the latest node-force vector for A-scan sonification."""
...
def sonify_ILM_RPE(f_ilm, f_rpe):
    """Send the separate retinal-boundary deformation forces."""
...
def compute_plate_ids(classes):
    """Group connected nodes of the same class into physical plates."""
    H, W = classes.shape
...
# Initialize SonoBox geometry and physical parameters before live force updates.
...
def start_recording(recording_path=None):
    """Ask SonoBox to begin writing the physical-model audio recording."""
    print("Starting recording...")
    transmitter.oscTransmittProc_RecordStart(recording_path)

def stop_recording():
    """Ask SonoBox to finish and close the current recording."""
