"""Track the needle tip using segmentations when available and optical flow otherwise."""
...
def extract_needle_keypoints(segmentation, tip_pos, n_shaft_points=20):
    """Sample stable keypoints along the segmented needle shaft, including the tip."""
    mask = segmentation == 1
    ys, xs = np.where(mask)
...
class NeedleTracker:
    """Maintain a smoothed needle estimate across successive OCT frames."""
    def __init__(
        self,
        init_frame=None,
        tip_pos=(0, 0),
    ...
    def update(self, frame, segmentation_map=None):
        """Prefer a segmentation update; otherwise advance existing points with LK flow."""
        if self.use_lk:
