"""Build the configuration and anatomical drivers for the physical sound model."""
...
# Base SonoBox configuration copied and specialized for the current node grid.
TEMPLATE_PATH = Path(__file__).resolve().parent / 'sonification_configs' / 'template.json'

# TODO: Very careful with listener placement
def compute_listeners(classes_per_patch, target_classes=[2,3]):
    """Place listeners near anatomical class transitions and lower grid corners."""
    ...
    # If no interfaces found, fallback to edge positions
    # listeners.append(f"m_{1}_{H//2}_0")  # Left edge, middle
    # listeners.append(f"m_{W-2}_{H//2}_0")  # Right edge, middle
    
    # # Add corner listeners as before
...
def sound_model_config_init(ROI, frame_roi, seg_roi, num_nodes_x=16, num_nodes_y=16, classes=None):
    """Create a scan-specific sound-model configuration from the JSON template."""
    """ 
    ...
    # Break ROI and segROI into patches so we have matrices (num_nodes_x, num_nodes_y, patch)
    # # TODO: Currently Not using patch info, we can use to remap to other param SPACE
    ...
    # Visualize Geometry
    # visualize_geometry(nodes, edges, springs)
...
def add_ILM_RPE_drivers(ilm_bound, rpe_bound, num_nodes_x, num_nodes_y, include_retina_in_ilm_rpe_drivers=False):
    """Assign simulator driver nodes to the detected ILM and RPE grid rows."""
...
def save_config(config, path):
    """Write the completed SonoBox configuration to disk."""
