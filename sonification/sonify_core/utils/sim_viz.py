"""Small visualization and run-output helpers shared by sonification scripts."""
...
class SonificationConfig:
    """Collect run settings and runtime arrays for JSON metadata."""
...
def draw_tracking_overlay(frame, tip, corr,debug=False):
    """Draw the tracked tip and correlation score when debugging is enabled."""
    if debug:
...
def show_if_debug(title, img, debug):
    """Show a blocking diagnostic image only in debug mode."""
    if debug:
...
def move_recording_to_run(recording_source, run_folder_path):
    """Move a completed WAV into its run folder and return the final path."""
    recording_source = os.path.abspath(recording_source)

    if not os.path.exists(recording_source):
        print(f"Recording not found: {recording_source}")
        return None

    recording_target = os.path.join(
        run_folder_path,
        "recording.wav",
    )

    if os.path.abspath(recording_source) != os.path.abspath(recording_target):
        shutil.move(recording_source, recording_target)

    print(f"Recording saved: {recording_target}")
