"""Process-management helper for shutting down the Processing/Java simulator."""
...
def kill_process_processing(simulator_process):
    """Terminate the simulator and its child process tree on Windows or Unix."""
    ...
    if os.name == "nt":
        try:
            subprocess.run(
                ["taskkill", "/PID", str(simulator_process.pid), "/T", "/F"],
                check=False,
