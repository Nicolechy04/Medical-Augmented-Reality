"""General force, ROI, tracking, deformation, and image-processing helpers."""
...
def load_force_data(folder):
    """Load the first RobotLog CSV found beside a scan."""
...
def get_rotated_ROI(ROI, M):
    """Map the corners of a rotated ROI back to original image coordinates."""
    x0, y0, side, _ = ROI
...
def compute_deflection_force_scaling(prev_frame, curr_frame, needle_tip_pos, rotated_patch_centers, 
    ...
    """Estimate a single force scale from intensity change inside a fixed ROI."""
...
def compute_needle_force_proxy(needle_speed, velocity_x, velocity_y):
    """Create a force-like record from needle motion when measured force is absent."""
...
def get_needle_tip_pos_from_seg(needle_seg, M=None):
    """Use the lowest needle-mask pixel as the tip and optionally transform it."""
...
def smoothen_and_assign_prev_position(prev_tip_pos, needle_tip_pos, alpha):
    """Smooth the tip while preventing backward motion in either image axis."""
...
def normalize_oct(img):
    """Remove OCT background bias, clip bright outliers, and normalize to [0, 1]."""
...
def normalize_oct_fast(img):
    """Percentile-based approximation of ``normalize_oct`` for faster processing."""
    gray = img.astype(np.float32)
    H, W, _ = gray.shape
...
def generate_attenuation_map_fast(img, window_size=5):
    """Estimate a normalized depth-attenuation map using rolling vertical sums."""
