Shader "VolumeRendering/DVR_Minimal_updated_v1" {
	Properties {
		[Header(Volume)]
		...
		[Header(Flow Advection NPR)]
		[Toggle(FLOW_ADVECTION)] FLOW_ADVECTION("Flow Advection (Tension)", Float) = 0
		[HideIfDisabled(FLOW_ADVECTION)] FlowNoiseTex("Flow Grain Noise (3D)", 3D) = "gray" {}
		[HideIfDisabled(FLOW_ADVECTION)] FlowNoiseScale("Flow Noise Scale", Range(1.0, 40.0)) = 12.0
		[HideIfDisabled(FLOW_ADVECTION)] FlowSpeed("Flow Speed", Range(0.0, 4.0)) = 1.2
		[HideIfDisabled(FLOW_ADVECTION)] FlowStrength("Flow Grain Strength", Range(0.0, 2.0)) = 1.0
	...
	SubShader {
		...
		Pass {
			...
			#pragma shader_feature FLOW_ADVECTION
			#pragma target 5.0
			...
			// 6-tap central difference — good enough for shading, avoids the kernel loop in computeGradientSmooth
			...
			fixed4 frag(v2f i) : SV_Target {
				...
				for (; travel > 0.0; travel -= StepSize, voxelCoord += deltaDir) {
					...
					// skip background voxels entirely — saves feature volume sampling in empty space
					if (sampleIntensity < 0.005) continue;

					if (sampleIntensity > IsoValue) {
						float alpha = saturate(sampleIntensity * StepSize * AlphaMultiplier);
						BaseColor.a = alpha;

						// use simple 6-tap gradient instead of computeGradientSmooth — the kernel loop there was ~30 samples per step
						float3 gradient = computeGradient(voxelCoord);
...
#if defined(SHOW_FEATURE_VOLUME)
					{
						...
						for (int s = 0; s < 8; s++) {
							...
							// direct lookup instead of sampleFeatureFiltered — cuts 6 extra taps per substep, trilinear HW does the filtering anyway
							float candidate = tex3D(FeatureVolumeTex, featureCoord).r;
...
#if defined(FLOW_ADVECTION)
							// Flow direction = gradient of the tension field, i.e. the
							// direction the retinal tissue is being dragged. A high-frequency
							// grain noise is advected along it so invisible tissue stress
							// becomes a visible animated flow; the scroll speed scales with
							// the local tension (still texture = no tension, fast = high).
							float3 fInv = getFeatureInvResolution();
							float tgx = tex3D(FeatureVolumeTex, voxelCoord + float3(fInv.x, 0, 0)).r
							          - tex3D(FeatureVolumeTex, voxelCoord - float3(fInv.x, 0, 0)).r;
							float tgy = tex3D(FeatureVolumeTex, voxelCoord + float3(0, fInv.y, 0)).r
							          - tex3D(FeatureVolumeTex, voxelCoord - float3(0, fInv.y, 0)).r;
							float tgz = tex3D(FeatureVolumeTex, voxelCoord + float3(0, 0, fInv.z)).r
							          - tex3D(FeatureVolumeTex, voxelCoord - float3(0, 0, fInv.z)).r;
							float3 flowDir = float3(tgx, tgy, tgz);
							float flowLen = length(flowDir);
							flowDir = flowLen > 1e-4 ? flowDir / flowLen : float3(0, 0, 0);

							// Scroll the noise sample opposite to flowDir so the pattern
							// appears to travel along flowDir over time.
							float3 noiseCoord = voxelCoord * FlowNoiseScale
							                  - flowDir * (_Time.y * FlowSpeed * (0.15 + featureLerp));
							float grain  = tex3D(FlowNoiseTex, noiseCoord).r;
							float streak = saturate((grain - 0.5) * (1.0 + 2.0 * FlowStrength) + 0.5);

							// Crests brighten, troughs darken → sand-like streaks flowing
							// through the blue→red heatmap.
							featureColorRGB *= (0.5 + 1.0 * streak);
							featureAlphaRamp *= (0.35 + 0.65 * streak);
#endif
