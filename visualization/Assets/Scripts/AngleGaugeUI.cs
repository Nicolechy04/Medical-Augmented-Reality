...
public class AngleGaugeUI : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // angle < 0 means no valid 2D reading — the angle < 0 check is sufficient.
        // Also hide if needle is not detected (ring hidden state: CannulaTipWorld == zero).
        bool needleNotDetected = Player.CannulaTipWorld == UnityEngine.Vector3.zero;
        if (angle < 0f || needleNotDetected)
        {
